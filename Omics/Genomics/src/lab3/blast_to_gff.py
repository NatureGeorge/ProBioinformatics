...
# "fieldnames" list beneath.
    def read_result(self):
        with open(self.input_file_name, 'r') as in_file:
            global fields
            fieldnames = ["header", "sstart", "sstop", "qheader",
                          "qlen", "qstart", "qstop", "sframe", "bitscore"]

            blast_reader = csv.DictReader(
                in_file, delimiter='\t', fieldnames=fieldnames)
            # This will be the list where each value is dict created from one line of input
            file_contents = []
            for i in blast_reader:
                file_contents.append(i)
            # set fields variable to number of fields read from input
...
# This method writes the output gff file
    def write_gff(self):
        ...
        with open(self.output_file_name, 'w+') as gff_out:
            fieldnames = ["header", "blastresult", "hit", "start",
                          "stop", "score", "orientation", "period", "ID"]
            read_results = self.read_result()
            gff_writer = csv.DictWriter(
                gff_out, delimiter='\t', fieldnames=fieldnames)
            iterator = 1
